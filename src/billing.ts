...
const billingKeyPair = readFileSync(path.resolve("pki/billing.key"));
...
app.use(async function(req, res, next) {
    ...
    try
    {
        ...
        .map(line => {
            ...
            let params: any = {
                'keychipid': 'A69E01A8888'
            };

            for (const kvp of line.split("&"))
            {
                let [key, val] = kvp.split("=");
...
app.post("/request/", function(req, res) {
    common.writeLog("Billing request: " + req.body, common.event.billing);
    ...
    try
    {
        ...
        console.log("Billing response: " + resItems, common.event.billing);
